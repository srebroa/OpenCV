...
def main():
    ...
    usbWebcam = cv2.VideoCapture(1)                     # declare a VideoCapture object and associate to webcam, 1=> use 2nd usb webcam!

    if usbWebcam.isOpened() == False:                           # check if VideoCapture object was associated to webcam successfully
        print "error: usbWebcam not accessed successfully\n\n"          # if not, print error message to std out
        os.system("pause")                                              # pause until user presses a key so user can see error message
        return                                                          # and exit function (which exits program)

    while cv2.waitKey(1) != 27 and usbWebcam.isOpened():                # until the Esc key is pressed or webcam connection is lost
        blnFrameReadSuccessfully, imgOriginal = usbWebcam.read()            # read next frame
        ...
        if circles is not None:                     # this line is necessary to keep program from crashing on next line if no circles were found
            for circle in circles[0]:                           # for each circle
                ...
                cv2.rectangle(imgOriginal, (x-radius, y-radius), (x+radius, y+radius), (255,0,0), 2) # draw blue rectangle around the detected object
            # end for
        # end if
